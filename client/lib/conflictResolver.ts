...
export class ConflictResolver {
  static detectConflicts(
    ...
    const hasGoalConflicts = this.hasArrayConflicts(
      localData.goals || [],
      cloudData.goals || [],
    );
    ...
    const hasConflict =
      hasGoalConflicts ||
      hasCompletedGoalConflicts ||
      hasPreferenceConflicts ||
      hasQuestConflicts;
  ...
  private static hasArrayConflicts(
    localArray: any[],
    cloudArray: any[],
  ): boolean {
    ...
    return commonIds.some((id) => {
      ...
      return (
        timeDiff > 60000 &&
        JSON.stringify(localItem) !== JSON.stringify(cloudItem)
      );
    });
  }

  private static hasPreferenceConflicts(
    localPrefs: any,
    cloudPrefs: any,
  ): boolean {
  ...
  private static hasQuestSystemConflicts(
    localQuest: any,
    cloudQuest: any,
  ): boolean {
    ...
    const levelDiff = Math.abs(
      (localQuest.level || 0) - (cloudQuest.level || 0),
    );
  ...
  private static performAutoMerge(
    ...
    return {
      ...
      preferences: {
        ...cloudData.preferences,
        theme:
          localData.preferences?.theme ||
          cloudData.preferences?.theme ||
          "system",
        notifications:
          localData.preferences?.notifications ??
          cloudData.preferences?.notifications ??
          true,
        reminderTime:
          localData.preferences?.reminderTime ||
          cloudData.preferences?.reminderTime,
        ...
        onboardingCompleted:
          ...
          false,
        useQuestSystem:
          cloudData.preferences?.useQuestSystem ??
          localData.preferences?.useQuestSystem ??
          true,
      },

      // Use the quest system data with higher level/XP
      questSystemData: this.getMostAdvancedQuestData(
        localData.questSystemData,
        cloudData.questSystemData,
      ),
    };
  ...
  private static getMostAdvancedQuestData(localData: any, cloudData: any): any {
    ...
    if (
      localLevel > cloudLevel ||
      (localLevel === cloudLevel && localXp > cloudXp)
    ) {
      return localData;
    }
