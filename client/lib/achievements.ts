...
export const ACHIEVEMENTS: Achievement[] = [
  ...
  },
  {
    ...
    rarity: 'rare',
    condition: (userData) => {
      const totalCompletions = userData.completedGoals?.reduce((sum: number, goal: any) => sum + (goal.completedCount || 0), 0) || 0;
  ...
  // Streak Achievements
  {
    ...
    rarity: 'common',
    condition: (userData) => {
      const maxStreak = Math.max(...(userData.goals?.map((g: any) => g.streak || 0) || [0]));
      return maxStreak >= 7;
    }
  },
  {
  ...
  // Consistency Achievements
  {
    ...
    rarity: 'rare',
    condition: (userData) => (userData.disciplineData?.consistencyScore || 0) >= 80
  },
  {
    id: 'consistency_95',
    title: 'Perfectionist',
    ...
    category: 'consistency',
    rarity: 'epic',
    condition: (userData) => (userData.disciplineData?.consistencyScore || 0) >= 95
  },
  ...
  // Recovery Achievements
  {
    ...
    rarity: 'common',
    condition: (userData) => {
      const maxCleanDays = Math.max(...(userData.addictions?.map((a: any) => a.cleanDays || 0) || [0]));
  ...
  // Special Achievements
  {
    ...
    category: 'goals',
    rarity: 'rare',
    condition: (userData) => (userData.goals?.filter((g: any) => !g.isCompleted).length || 0) >= 3
  },
  {
    id: 'balanced_life',
    title: 'Balanced Life',
    ...
    rarity: 'epic',
    condition: (userData) => {
      const categories = new Set(userData.goals?.filter((g: any) => !g.isCompleted).map((g: any) => g.category) || []);
...
export function checkAchievements(userData: any, previousUserData?: any): Achievement[] {
  ...
  ACHIEVEMENTS.forEach(achievement => {
    const wasEarned = userData.achievements?.some((a: any) => a.id === achievement.id);
...
export function getEarnedAchievements(userData: any): Achievement[] {
  return ACHIEVEMENTS.filter(achievement => 
    ...
    ...achievement,
    earnedAt: userData.achievements?.find((a: any) => a.id === achievement.id)?.earnedAt
  }));
}
...
export function getRarityBorder(rarity: Achievement['rarity']): string {
  switch (rarity) {
    ...
    case 'rare': return 'border-blue-200 dark:border-blue-700';
    case 'epic': return 'border-purple-200 dark:border-purple-700';
    case 'legendary': return 'border-yellow-200 dark:border-yellow-700';
