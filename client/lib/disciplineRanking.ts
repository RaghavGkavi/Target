...
export function calculateDisciplineRank(
  ...
  const totalStreaks = activeGoals.reduce(
    (sum: number, goal: any) => sum + (goal.streak || 0),
    0,
  );
  ...
  const completedCycles = completedGoals.reduce(
...
export function calculateConsistencyScore(goals: any[]): number {
  ...
  const totalPossibleDays = goals.reduce(
    (sum, goal) => sum + goal.targetDays,
    0,
  );
  const totalCompletedDays = goals.reduce(
    (sum, goal) => sum + goal.daysCompleted,
    0,
  );
...
export function getNextRankInfo(currentRank: DisciplineRank) {
  ...
  const pointsNeeded = Math.ceil(
    (currentIndex + 2) * 6.67 - (currentIndex + 1) * 6.67,
  );
