...
export class DataSyncService {
  ...
  static async saveUserDataToCloud(userId: string, userData: UserData): Promise<void> {
    try {
      ...
      if (cloudData.goals) {
        cloudData.goals = cloudData.goals.map(goal => ({
          ...goal,
          lastUpdated: goal.lastUpdated ? Timestamp.fromDate(goal.lastUpdated) : null
        }));
      }
      ...
      if (cloudData.completedGoals) {
        cloudData.completedGoals = cloudData.completedGoals.map(goal => ({
          ...goal,
          completionDates: goal.completionDates?.map((date: Date) => Timestamp.fromDate(date))
        }));
      }
      ...
      if (cloudData.questSystemData) {
        ...
        if (qsd.currentQuests) {
          qsd.currentQuests = qsd.currentQuests.map(quest => ({
            ...
            dateCompleted: quest.dateCompleted ? Timestamp.fromDate(quest.dateCompleted) : undefined
          }));
        }
        ...
        if (qsd.weeklyStats?.lastStreakDate) {
          qsd.weeklyStats.lastStreakDate = Timestamp.fromDate(qsd.weeklyStats.lastStreakDate);
        }

        if (qsd.dailyStats) {
          qsd.dailyStats.date = Timestamp.fromDate(qsd.dailyStats.date);
          qsd.dailyStats.lastUpdated = Timestamp.fromDate(qsd.dailyStats.lastUpdated);
        }
      }
    ...
    try {
      ...
      if (userData.goals) {
        userData.goals = userData.goals.map(goal => ({
          ...goal,
          lastUpdated: goal.lastUpdated ? goal.lastUpdated.toDate() : new Date()
        }));
      }
      ...
      if (userData.completedGoals) {
        userData.completedGoals = userData.completedGoals.map(goal => ({
          ...goal,
          completionDates: goal.completionDates?.map((timestamp: any) => timestamp.toDate())
        }));
      }
      ...
      if (userData.questSystemData) {
        ...
        if (qsd.currentQuests) {
          qsd.currentQuests = qsd.currentQuests.map(quest => ({
            ...
            dateCompleted: quest.dateCompleted ? quest.dateCompleted.toDate() : undefined
          }));
        }
        ...
        if (qsd.weeklyStats?.lastStreakDate) {
          qsd.weeklyStats.lastStreakDate = qsd.weeklyStats.lastStreakDate.toDate();
        }
  ...
  private static mergeArraysByProperty(localArray: any[], cloudArray: any[], property: string): any[] {
    const merged = [...cloudArray];
