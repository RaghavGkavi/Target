...
interface SyncConflictDialogProps {
  ...
  localData: UserData;
  cloudData: UserData;
  onResolve: (resolvedData: UserData, strategy: "local" | "cloud" | "merge") => void;
}
...
export function SyncConflictDialog({
  ...
  const [selectedStrategy, setSelectedStrategy] = useState<"local" | "cloud" | "merge">("merge");
    ...
    switch (selectedStrategy) {
      ...
      default:
        resolvedData = {
          ...
          completedGoals: mergeArraysById(localData.completedGoals || [], cloudData.completedGoals || []),
          addictions: mergeArraysById(localData.addictions || [], cloudData.addictions || []),
          achievements: mergeArraysById(localData.achievements || [], cloudData.achievements || []),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Your data differs between this device and the cloud. Choose how to resolve the conflict.
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs value={selectedStrategy} onValueChange={(value) => setSelectedStrategy(value as any)}>
          ...
          <TabsContent value="merge" className="space-y-4">
            <div className="text-sm text-muted-foreground">
              Recommended: Combines data from both sources intelligently, keeping the best of both.
            </div>
          ...
          <TabsContent value="cloud" className="space-y-4">
            <div className="text-sm text-muted-foreground">
              Replace local data with cloud data. Any local-only changes will be lost.
            </div>
            <DataSummaryCard 
              title="Cloud Data" 
              icon={<Cloud className="h-4 w-4" />}
          ...
          <TabsContent value="local" className="space-y-4">
            <div className="text-sm text-muted-foreground">
              Keep local data and overwrite cloud. Data from other devices will be lost.
            </div>
            <DataSummaryCard 
              title="Local Data" 
              icon={<Smartphone className="h-4 w-4" />}
        ...
        <DialogFooter>
          ...
          <Button onClick={handleResolve}>
            {selectedStrategy === "merge" ? "Merge Data" : 
             selectedStrategy === "cloud" ? "Use Cloud Data" : "Use Local Data"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
      ...
      <div className="space-y-2 text-sm">
        ...
        {summary.questLevel > 0 && (
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline">Lv.{summary.questLevel} ({summary.xp} XP)</Badge>
          </div>
        )}
      </div>
    </div>
