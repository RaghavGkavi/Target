...
export default function CompletedGoals() {
  ...
  const getLevelBadge = (level: number) => {
    if (level >= 5) return { icon: Crown, color: "text-yellow-500", label: "Master" };
    if (level >= 4) return { icon: Medal, color: "text-purple-500", label: "Expert" };
    if (level >= 3) return { icon: Star, color: "text-blue-500", label: "Advanced" };
    if (level >= 2) return { icon: Trophy, color: "text-green-500", label: "Intermediate" };
  ...
  const totalDaysAchieved = completedGoals.reduce((sum, goal) => sum + goal.totalDaysCompleted, 0);
  const longestOverallStreak = Math.max(...completedGoals.map(goal => goal.longestStreak));
      ...
      <main className="container mx-auto px-4 py-6 space-y-8">
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Total Days Achieved</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary">{completedGoals.length} goals mastered</Badge>
          </div>
            ...
            <Card className="rounded-xl border-dashed border-2">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-muted-foreground mb-6">
                  Complete your first goal to see it appear here and start building your achievement history!
                </p>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div className="space-y-1">
                          <p className="text-lg font-bold text-warning">{goal.completedCount}</p>
                          <p className="text-xs text-muted-foreground">Completions</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-lg font-bold text-success">{goal.totalDaysCompleted}</p>
                          <p className="text-xs text-muted-foreground">Total Days</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-lg font-bold text-streak">{goal.longestStreak}</p>
                      ...
                      <div className="space-y-2">
                        ...
                        <Progress 
                          value={Math.min(100, (goal.completedCount * 20) % 100)} 
                          className="h-2" 
                        />
                        <p className="text-xs text-muted-foreground">
                          {5 - (goal.completedCount % 5)} more completions to next level
                        </p>
                      </div>
          ...
          <Card className="rounded-xl bg-gradient-to-r from-primary/10 to-accent/10 border-primary/20">
            <CardContent className="p-6">
              <div className="text-center space-y-4">
                ...
                <div>
                  <h3 className="text-xl font-bold mb-2">Incredible Progress!</h3>
                  <p className="text-muted-foreground mb-4">
                    You've completed {totalCompletions} goals and built {totalDaysAchieved} days of positive habits. 
